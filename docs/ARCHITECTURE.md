...
- 图像库快照按任意深度集合递归构建 ID 索引；常驻导航通过随机相册和阅读摘要接口获取单项/计数，首页通过仪表盘接口获取未读计数、有限预览和在读摘要，均避免下载全量相册。完整未读由服务端按“目录 revision + 活动版本”缓存派生索引并分页返回，同一版本的后续页不重复扫描全库；最近、收藏等列表仍按页面需要查询摘要。
