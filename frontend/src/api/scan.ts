...
export interface AlbumSummary {
  ...
  files?: string[];
  imageFiles?: string[];
  /** 视频文件 ID 列表（imageCount=0 时 coverImage 指向这里）。 */
  videoFiles?: string[];
  author?: string;
  modTime?: string;
  folderSize?: number;
...
export const scanApi = {
  ...
  result: (id: string) =>
    api<{ ok: boolean; result: ScanResult }>(`/api/scans/${id}`),
  cancel: (id: string) =>
    api<{ ok: boolean }>(`/api/scans/${id}`, { method: "DELETE" }),
  // 拿后端磁盘缓存的上次扫描结果；首次启动 / 还没扫过时 404,调用方应吞掉。
  latest: () => api<{ ok: boolean; result: ScanResult }>("/api/library"),
